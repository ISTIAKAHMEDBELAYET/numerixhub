...
// Allowlist of all valid calculator slugs — prevents clashes with existing
// top-level routes such as /about, /api, /calculators, /contact, etc.
...
export async function generateMetadata({ params }: { params: { slug: string } }) {
  if (!CALCULATOR_SLUGS.has(params.slug)) return { title: 'Calculator Not Found' };
...
export default function CalculatorPage({ params }: { params: { slug: string } }) {
  if (!CALCULATOR_SLUGS.has(params.slug)) notFound();
  const calc = getCalculatorBySlug(params.slug);
  if (!calc) notFound();
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: calc.name,
        item: `https://numerixhub.pages.dev/${calc.slug}/`,
      },
    ],
  };
