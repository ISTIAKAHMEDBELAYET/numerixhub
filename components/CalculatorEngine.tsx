...
function ScientificCalculator() {
  ...
  return (
    <div className="w-full max-w-none sm:max-w-md lg:max-w-xl mx-auto px-0 sm:px-0">
      <div className="bg-gray-900 text-white rounded-2xl p-2 sm:p-6 shadow-xl">
        <div className="bg-black/50 rounded-xl p-2 sm:p-5 mb-3 sm:mb-4 min-h-[64px] sm:min-h-[96px] flex items-end justify-end">
          <div className="text-right w-full">
            <div className="text-gray-400 text-xs sm:text-base truncate">{expr || '0'}</div>
            <div className="text-xl sm:text-4xl font-bold truncate">{display}</div>
          </div>
        </div>
        <div className="space-y-1.5 sm:space-y-3">
          {rows.map((row, ri) => (
            <div key={ri} className={`grid gap-1.5 sm:gap-3 ${row.length === 1 ? 'grid-cols-1' : 'grid-cols-5'}`}>
              {row.map((btn) => {
                ...
                return (
                  <button
                    ...
                    className={`min-w-0 py-2 sm:py-4 rounded-xl text-[11px] sm:text-xl leading-tight font-bold transition-all active:scale-95 ${
...
const calcContent: Record<string, { howTo: string; formula?: string; faqs: { q: string; a: string }[]; sections?: { title: string; content: string }[] }> = {
  ...
  'time-duration-calculator': {
    howTo: 'Enter start and end date-time values (or mixed units) to compute elapsed duration in days, hours, minutes, and seconds.',
    formula: 'Duration = end timestamp - start timestamp, then decomposed into desired time units.',
    faqs: [
      { q: 'What does time duration calculator do?', a: 'It calculates elapsed time between two points.' },
      { q: 'Can it handle cross-midnight intervals?', a: 'Yes, date-aware subtraction handles overnight spans.' },
      { q: 'Do leap years matter?', a: 'Yes, calendar-aware calculations account for varying month/day lengths.' },
      { q: 'Can daylight saving changes affect result?', a: 'Yes, timezone-aware mode can reflect DST transitions.' },
      { q: 'Should timezone be specified?', a: 'For precise cross-region timing, timezone context is important.' },
      { q: 'Can output be shown in total hours only?', a: 'Yes, many tools provide both decomposed and total-unit outputs.' },
      { q: 'Is duration inclusive of both endpoints?', a: 'It depends on mode; clarify inclusive/exclusive rule for your use case.' },
      { q: 'Can negative duration occur?', a: 'Yes, if start is after end in raw subtraction mode.' },
      { q: 'Can this help payroll windows?', a: 'Yes, it is useful for shift and task-time reconciliation.' },
      { q: 'Is this scheduling app replacement?', a: 'No, it is a focused time-difference calculator.' },
    ],
    sections: [
      { title: 'Timestamp Arithmetic', content: 'Duration tools compute differences between temporal reference points.' },
      { title: 'Calendar Nuance', content: 'Month length, leap years, and DST influence accurate results.' },
      { title: 'Output Flexibility', content: 'Results can be represented in composite or single-unit formats.' },
      { title: 'Workflow Utility', content: 'Common in planning, payroll, and SLA tracking contexts.' },
      { title: 'Interpretation Settings', content: 'Inclusive/exclusive endpoint choice should match business rules.' },
    ],
  },
  'day-counter': {
    howTo: 'Enter two dates to count total days between them, optionally including business-day or inclusive-range options.',
    formula: 'Day count = date2 - date1 in calendar days, with optional inclusive adjustment.',
    faqs: [
      { q: 'What is day counter used for?', a: 'It counts calendar-day gaps between dates for planning and deadlines.' },
      { q: 'Can weekends be excluded?', a: 'Some modes support business-day counting excluding weekends/holidays.' },
      { q: 'Does leap day affect counts?', a: 'Yes, leap years add an extra calendar day where applicable.' },
      { q: 'What is inclusive counting?', a: 'Inclusive mode counts both start and end dates in total.' },
      { q: 'Can past and future dates both work?', a: 'Yes, counters can handle either direction.' },
      { q: 'Can this track project timelines?', a: 'Yes, it helps estimate elapsed and remaining day windows.' },
      { q: 'Are holidays automatically handled?', a: 'Only if holiday calendars are explicitly included.' },
      { q: 'Can negative values appear?', a: 'Yes, if order implies reverse-direction counting.' },
      { q: 'Is this legal deadline tool?', a: 'No, formal legal timelines may have jurisdiction-specific rules.' },
      { q: 'Does timezone matter for date-only mode?', a: 'Usually minimal, but date boundaries can shift in cross-zone contexts.' },
    ],
    sections: [
      { title: 'Calendar Distance', content: 'Day counting provides simple timeline quantification between dates.' },
      { title: 'Business vs Calendar Days', content: 'Use the mode aligned with operational or legal requirements.' },
      { title: 'Inclusive Logic Choice', content: 'Endpoint-count policy can change totals by one day.' },
      { title: 'Planning Applications', content: 'Useful for milestones, countdowns, and compliance windows.' },
      { title: 'Context Limits', content: 'Holiday treatment and jurisdictional rules may require customization.' },
    ],
  },
  'day-of-the-week-calculator': {
    howTo: 'Enter a date to determine the weekday (Monday-Sunday) for scheduling, historical lookup, or recurrence checks.',
    formula: 'Weekday computed from calendar-date indexing algorithms over Gregorian rules.',
    faqs: [
      { q: 'What does day-of-week calculator provide?', a: 'It maps a calendar date to its corresponding weekday.' },
      { q: 'Can it handle historical dates?', a: 'Yes, within supported calendar-system assumptions.' },
      { q: 'Does leap year affect weekday?', a: 'Yes, leap-day structure shifts weekday offsets across years.' },
      { q: 'Can this help event planning?', a: 'Yes, it is useful for anniversaries and scheduling patterns.' },
      { q: 'Is Gregorian calendar always assumed?', a: 'Most calculators use Gregorian rules unless specified otherwise.' },
      { q: 'Can weekdays repeat same date annually?', a: 'They shift each year based on leap-year progression.' },
      { q: 'Can this verify recurring billing dates?', a: 'Yes, weekday checks help validate periodic schedules.' },
      { q: 'Does timezone affect date-only weekday?', a: 'Date interpretation near midnight across zones may change practical mapping.' },
      { q: 'Can invalid dates be entered?', a: 'Robust tools should validate month/day combinations.' },
      { q: 'Is this chronology research replacement?', a: 'No, it is a fast weekday lookup aid.' },
    ],
    sections: [
      { title: 'Weekday Mapping', content: 'Date-to-weekday conversion supports scheduling and record interpretation.' },
      { title: 'Leap-Year Offsets', content: 'Calendar leap structure drives annual weekday drift.' },
      { title: 'Historical Utility', content: 'Helpful for event backtracking and archival timeline checks.' },
      { title: 'Validation Role', content: 'Weekday checks can catch date-entry mistakes in workflows.' },
      { title: 'Calendar Assumption', content: 'Results depend on the underlying calendar system implemented.' },
    ],
  },
  'mortgage-calculator-uk': {
    howTo: 'Enter property price, deposit, interest rate, and term to estimate UK mortgage monthly payments and borrowing profile.',
    formula: 'Monthly payment derived from standard amortization relation using principal, rate, and term.',
    faqs: [
      { q: 'How is UK mortgage payment calculated?', a: 'It uses loan amount, interest rate, and repayment term through amortization math.' },
      { q: 'Does deposit size affect rate offers?', a: 'Yes, higher deposit often improves LTV and potential pricing.' },
      { q: 'What is LTV?', a: 'Loan-to-value ratio compares mortgage amount to property value.' },
      { q: 'Can fixed and variable rates be compared?', a: 'Yes, scenario comparison helps evaluate payment stability vs flexibility.' },
      { q: 'Should fees be included?', a: 'Yes, arrangement and legal costs matter for total borrowing economics.' },
      { q: 'Does stamp duty affect affordability?', a: 'Yes, transaction taxes can significantly impact upfront cash need.' },
      { q: 'Can overpayments reduce term?', a: 'Yes, extra principal often reduces total interest and payoff horizon.' },
      { q: 'Is this lender decision tool?', a: 'No, lenders apply full affordability and credit assessments.' },
      { q: 'Can rate resets change monthly payment?', a: 'Yes, variable/tracker products can reprice over time.' },
      { q: 'Should stress rates be considered?', a: 'Yes, stress testing improves resilience to rate increases.' },
    ],
    sections: [
      { title: 'UK Borrowing Framework', content: 'Core mortgage math should be interpreted with UK-specific cost factors.' },
      { title: 'LTV Sensitivity', content: 'Deposit ratio drives risk tier and potential pricing outcomes.' },
      { title: 'Upfront Cost Planning', content: 'Stamp duty and fees meaningfully affect cash-on-completion.' },
      { title: 'Rate-Risk Management', content: 'Scenario analysis is important for variable-rate exposure.' },
      { title: 'Affordability Discipline', content: 'Use conservative assumptions before committing to long-term debt.' },
    ],
  },
  'canadian-mortgage-calculator': {
    howTo: 'Enter home price, down payment, mortgage rate, and amortization to estimate Canadian mortgage payment and loan profile.',
    formula: 'Canadian payment estimates commonly account for local compounding/payment conventions and insured-loan premiums when applicable.',
    faqs: [
      { q: 'How does Canadian mortgage differ in calculation?', a: 'Compounding/payment conventions and insurance rules can differ from other markets.' },
      { q: 'When is mortgage default insurance required?', a: 'Lower down-payment scenarios may require insured-mortgage premiums.' },
      { q: 'What is amortization period?', a: 'It is total repayment horizon used to compute scheduled payments.' },
      { q: 'Can term length differ from amortization?', a: 'Yes, term renewals can occur within a longer amortization schedule.' },
      { q: 'Should closing costs be included?', a: 'Yes, legal and transfer costs affect total purchase budget.' },
      { q: 'Can prepayment privileges reduce cost?', a: 'Yes, permitted extra payments can lower interest burden.' },
      { q: 'What happens at renewal?', a: 'Rate and payment conditions may change based on market and lender terms.' },
      { q: 'Does rate stress testing matter?', a: 'Yes, qualification stress tests support repayment resilience.' },
      { q: 'Can this replace lender quote?', a: 'No, final offers depend on underwriting and product details.' },
      { q: 'Should property tax be modeled separately?', a: 'Yes, total housing cost includes taxes, insurance, and other recurring expenses.' },
    ],
    sections: [
      { title: 'Canada-Specific Mechanics', content: 'Payment modeling should reflect local mortgage and insurance conventions.' },
      { title: 'Amortization vs Term', content: 'Renewal cycles can alter financing trajectory before full amortization end.' },
      { title: 'Insurance Premium Impact', content: 'Insured-loan premiums can materially change effective borrowing cost.' },
      { title: 'Renewal Risk Awareness', content: 'Future rates at renewal are a key long-term affordability factor.' },
      { title: 'Full-Cost Perspective', content: 'Mortgage payment should be viewed alongside complete ownership expenses.' },
    ],
  },
  'mortgage-amortization-calculator': {
    howTo: 'Enter loan amount, interest rate, term, and optional extra payments to generate amortization schedule and interest/principal breakdown.',
    formula: 'Monthly payment from amortization formula; each period splits payment into interest and principal based on remaining balance.',
    faqs: [
      { q: 'What is amortization schedule?', a: 'A timeline showing payment-by-payment principal, interest, and remaining balance.' },
      { q: 'Why is early mortgage mostly interest?', a: 'Interest is computed on highest outstanding balance at loan start.' },
      { q: 'Can extra payments shorten term?', a: 'Yes, additional principal accelerates payoff and lowers total interest.' },
      { q: 'Does fixed rate keep payment constant?', a: 'Principal+interest portion stays constant in standard fixed-rate loans.' },
      { q: 'Can schedule help refinance decisions?', a: 'Yes, it clarifies remaining interest burden and break-even analysis.' },
      { q: 'What if payment date changes?', a: 'Timing can shift interest accrual depending on servicer policy.' },
      { q: 'Can biweekly plans be modeled?', a: 'Yes, frequent-payment strategies can be approximated for savings impact.' },
      { q: 'Should escrow be part of amortization?', a: 'Escrow affects cash outflow but not loan principal math directly.' },
      { q: 'Can this show cumulative interest?', a: 'Yes, cumulative totals are key output for cost comparison.' },
      { q: 'Is this legal lender statement?', a: 'No, final records come from loan servicer statements.' },
    ],
    sections: [
      { title: 'Payment Composition', content: 'Amortization reveals changing interest-principal share over time.' },
      { title: 'Cost Transparency', content: 'Schedule output clarifies full borrowing cost trajectory.' },
      { title: 'Prepayment Strategy', content: 'Extra principal effects can be quantified before committing.' },
      { title: 'Decision Support', content: 'Useful for refinance, sell-vs-hold, and cash-flow planning.' },
      { title: 'Servicing Alignment', content: 'Real statements may differ slightly due to timing and rounding rules.' },
    ],
  },
  'percent-off-calculator': {
    howTo: 'Enter original price and discount percentage to compute sale price, amount saved, and optional tax-adjusted total.',
    formula: 'Discounted price = original price * (1 - discount%).',
    faqs: [
      { q: 'How is percent off calculated?', a: 'Subtract percentage of original price from original price.' },
      { q: 'Can multiple discounts be combined directly?', a: 'Sequential discounts are multiplicative, not simple addition.' },
      { q: 'Does tax apply before or after discount?', a: 'Usually tax applies to discounted subtotal, depending on jurisdiction.' },
      { q: 'How much did I save?', a: 'Savings equals original price minus final discounted price.' },
      { q: 'Can coupon and sale stack?', a: 'If allowed, apply in sequence per store policy.' },
      { q: 'Why two 50% discounts are not 100% off?', a: 'Second discount applies to already reduced price.' },
      { q: 'Can this compare offers?', a: 'Yes, useful for evaluating competing promotional structures.' },
      { q: 'Is percentage always on MSRP?', a: 'Not always; retailer may discount from current listed price.' },
      { q: 'Can rounding affect final cents?', a: 'Yes, checkout rounding and tax rules can alter cents.' },
      { q: 'Is this receipt-accurate for all stores?', a: 'It is an estimate unless exact store rules are modeled.' },
    ],
    sections: [
      { title: 'Discount Arithmetic', content: 'Percent-off calculations translate promotions into clear price outcomes.' },
      { title: 'Sequential Rule Awareness', content: 'Stacked discounts require ordered multiplicative application.' },
      { title: 'Tax Interaction', content: 'After-discount taxation rules influence final payable totals.' },
      { title: 'Offer Comparison', content: 'Normalized savings metrics improve purchasing decisions.' },
      { title: 'Checkout Variance', content: 'Store rounding and policy differences can create small deviations.' },
    ],
  },
  'anorexic-bmi-calculator': {
    howTo: 'Enter height and weight to compute BMI and view underweight-risk context with strong emphasis on clinical follow-up.',
    formula: 'BMI = weight(kg) / height(m)^2, interpreted with severe-underweight caution ranges.',
    faqs: [
      { q: 'What is this BMI variant for?', a: 'It highlights low-BMI risk context and encourages medical evaluation.' },
      { q: 'Can BMI diagnose eating disorders?', a: 'No, diagnosis requires comprehensive clinical assessment.' },
      { q: 'Why is low BMI concerning?', a: 'Persistent low BMI may relate to nutritional deficiency and health complications.' },
      { q: 'Can normal BMI still hide disorder risk?', a: 'Yes, behavioral and psychological symptoms can exist at any BMI.' },
      { q: 'Should this be used for self-diagnosis?', a: 'No, use as screening information only and seek professional guidance.' },
      { q: 'Can athletes have unusual BMI patterns?', a: 'Yes, body composition differences can affect interpretation.' },
      { q: 'What signs require urgent help?', a: 'Rapid weight loss, weakness, fainting, or severe restriction need prompt care.' },
      { q: 'Can teens use adult BMI cutoffs?', a: 'Pediatric assessment should use age/sex-specific growth charts.' },
      { q: 'How often should BMI be checked?', a: 'Frequent self-checking may be unhelpful; follow clinician guidance.' },
      { q: 'Is this a treatment plan?', a: 'No, treatment planning must be done by qualified professionals.' },
    ],
    sections: [
      { title: 'Risk-Screening Context', content: 'Low-BMI outputs should be interpreted as caution flags, not diagnoses.' },
      { title: 'Clinical Priority', content: 'Medical and mental-health evaluation is essential when risk signs appear.' },
      { title: 'Holistic Assessment', content: 'Behavioral, nutritional, and physiological factors matter beyond BMI.' },
      { title: 'Sensitive Use', content: 'Tools should avoid reinforcing harmful comparison behavior.' },
      { title: 'Support Pathways', content: 'Early professional support improves outcomes and safety.' },
    ],
  },
  'weight-watchers-points-calculator': {
    howTo: 'Enter nutritional values (calories, fat, protein, etc., depending on points system) to estimate points equivalent.',
    formula: 'Points estimation uses system-specific weighted nutrition coefficients and rounding rules.',
    faqs: [
      { q: 'What are points in diet programs?', a: 'Points simplify food tracking by converting nutrition into a budget metric.' },
      { q: 'Do formulas vary by program version?', a: 'Yes, official formulas and categories can change over time.' },
      { q: 'Can homemade meals be estimated?', a: 'Yes, if ingredient nutrition totals are entered accurately.' },
      { q: 'Why can app and calculator differ?', a: 'Different formula versions or rounding rules can produce differences.' },
      { q: 'Do zero-point foods affect totals?', a: 'Program rules may classify certain foods differently from raw formulas.' },
      { q: 'Can points replace full nutrition awareness?', a: 'No, overall quality and micronutrient balance still matter.' },
      { q: 'Is protein always reducing points?', a: 'It depends on the specific scoring model and version.' },
      { q: 'Can portion size errors distort points?', a: 'Yes, measurement accuracy is critical for reliable tracking.' },
      { q: 'Should activity points be included?', a: 'Only if your tracking framework explicitly uses them.' },
      { q: 'Is this official program backend?', a: 'No, it is an estimation aid.' },
    ],
    sections: [
      { title: 'Simplified Tracking Model', content: 'Points systems convert complex nutrition into manageable daily budgets.' },
      { title: 'Version Dependence', content: 'Interpretation should match the exact program rules in use.' },
      { title: 'Input Accuracy', content: 'Reliable portion and nutrition data are essential for consistency.' },
      { title: 'Behavioral Utility', content: 'Point budgeting can improve adherence for many users.' },
      { title: 'Nutrition Balance', content: 'Point targets should still be paired with food-quality considerations.' },
    ],
  },
  'overweight-calculator': {
    howTo: 'Enter height and weight to assess BMI category and estimate amount above selected healthy-range thresholds.',
    formula: 'BMI-based comparison against reference ranges to estimate excess-weight magnitude.',
    faqs: [
      { q: 'What does overweight calculator show?', a: 'It estimates BMI category and approximate weight above reference range.' },
      { q: 'Is BMI enough for health judgment?', a: 'No, body composition and clinical markers provide fuller context.' },
      { q: 'Can muscular people be misclassified?', a: 'Yes, high lean mass can elevate BMI without excess body fat.' },
      { q: 'Should one target number be used?', a: 'Range-based targets are often more practical and sustainable.' },
      { q: 'Can small changes improve health?', a: 'Yes, modest, consistent improvements can meaningfully reduce risk.' },
      { q: 'How fast should weight loss be?', a: 'Gradual approaches are generally safer and more sustainable.' },
      { q: 'Does waist circumference matter?', a: 'Yes, abdominal fat distribution is an important risk indicator.' },
      { q: 'Can medications affect weight trends?', a: 'Yes, some conditions and medications influence body weight.' },
      { q: 'Is this a medical diagnosis?', a: 'No, it is a screening and planning tool.' },
      { q: 'Should goals be personalized?', a: 'Yes, personalized plans with professionals usually work best.' },
    ],
    sections: [
      { title: 'Screening Orientation', content: 'Overweight estimates provide directional context, not definitive diagnosis.' },
      { title: 'Risk Context Expansion', content: 'Waist, labs, and fitness metrics should complement BMI-based outputs.' },
      { title: 'Sustainable Change', content: 'Incremental habits generally outperform extreme short-term strategies.' },
      { title: 'Personalization Need', content: 'Individual history and constraints should guide target setting.' },
      { title: 'Clinical Collaboration', content: 'Professional support improves safety and long-term success.' },
    ],
  },
  'prime-factorization-calculator': {
    howTo: 'Enter a positive integer to decompose it into prime factors and view exponent form representation.',
    formula: 'Prime factorization expresses n as product of primes: n = p1^a1 * p2^a2 * ...',
    faqs: [
      { q: 'What is prime factorization?', a: 'It breaks a number into multiplication of prime numbers only.' },
      { q: 'Why is prime factorization useful?', a: 'It supports GCF, LCM, simplification, and number-theory analysis.' },
      { q: 'Can every integer be factorized uniquely?', a: 'Yes, uniqueness holds up to ordering by fundamental theorem of arithmetic.' },
      { q: 'Is 1 prime?', a: 'No, 1 is neither prime nor composite.' },
      { q: 'Can large numbers be slow to factor?', a: 'Yes, factorization complexity rises significantly with large inputs.' },
      { q: 'Do repeated factors use exponents?', a: 'Yes, repeated prime factors are compactly shown in exponent form.' },
      { q: 'Can negative numbers be factorized?', a: 'Yes, with -1 separated and factorization of absolute value.' },
      { q: 'Is this same as factoring polynomials?', a: 'No, this tool addresses integer factorization.' },
      { q: 'Can this help cryptography learning?', a: 'Yes, prime factorization concepts are foundational in crypto theory.' },
      { q: 'Does calculator prove primality?', a: 'It can indicate prime outcomes but formal primality tests may differ.' },
    ],
    sections: [
      { title: 'Canonical Decomposition', content: 'Prime factorization gives a unique multiplicative fingerprint of integers.' },
      { title: 'Arithmetic Utility', content: 'Many integer operations become easier in prime-exponent form.' },
      { title: 'Complexity Considerations', content: 'Factoring hard cases can be computationally expensive.' },
      { title: 'Educational Relevance', content: 'Critical for understanding divisibility and theorem-based proofs.' },
      { title: 'Applied Context', content: 'Integer factorization ideas appear in security and algorithm design.' },
    ],
  },
  'common-factor-calculator': {
    howTo: 'Enter two or more integers to list shared factors and identify greatest common factor for simplification tasks.',
    formula: 'Common factors are integers dividing all inputs; largest is GCF.',
    faqs: [
      { q: 'What is a common factor?', a: 'A number that divides each input integer without remainder.' },
      { q: 'How is common-factor list useful?', a: 'It helps simplify ratios, fractions, and divisibility problems.' },
      { q: 'Is common factor same as common multiple?', a: 'No, factors divide numbers while multiples are produced from them.' },
      { q: 'Can there be only one common factor?', a: 'Yes, coprime sets share only 1.' },
      { q: 'Do negative numbers change factors?', a: 'Common-factor logic typically uses absolute values for divisor lists.' },
      { q: 'Can this return all common factors?', a: 'Yes, not just the greatest one.' },
      { q: 'Why does GCF matter most?', a: 'It gives maximal simplification in many arithmetic contexts.' },
      { q: 'Can zero be included?', a: 'Handling zero depends on convention and companion inputs.' },
      { q: 'Is this useful for algebra?', a: 'Yes, numeric common factors mirror factoring patterns in expressions.' },
      { q: 'Can large input sets be processed?', a: 'Yes, via iterative GCF and divisor filtering methods.' },
    ],
    sections: [
      { title: 'Shared Divisibility Map', content: 'Common factors reveal overlap in integer structure.' },
      { title: 'Simplification Workflow', content: 'Factor intersections support reduction and normalization steps.' },
      { title: 'GCF Focus', content: 'Greatest common factor is often the most actionable output.' },
      { title: 'Algorithmic Efficiency', content: 'Efficient methods avoid brute-force scanning for large values.' },
      { title: 'Cross-Domain Use', content: 'Useful in arithmetic, algebra, and ratio applications.' },
    ],
  },
  'basic-calculator': {
    howTo: 'Enter numbers and operators to perform standard arithmetic operations including addition, subtraction, multiplication, and division.',
    formula: 'Basic arithmetic follows operator precedence where supported: multiplication/division before addition/subtraction.',
    faqs: [
      { q: 'What operations are supported?', a: 'Typically +, -, *, / and sometimes percent and memory functions.' },
      { q: 'Does order of operations matter?', a: 'Yes, expression evaluation may follow precedence rules.' },
      { q: 'What happens when dividing by zero?', a: 'Division by zero is undefined and should produce an error.' },
      { q: 'Can decimals be used?', a: 'Yes, floating-point input is standard.' },
      { q: 'Why can repeated decimals round oddly?', a: 'Binary floating-point representation can cause tiny precision artifacts.' },
      { q: 'Is this same as scientific calculator?', a: 'No, scientific tools include advanced functions beyond basic arithmetic.' },
      { q: 'Can keyboard input be used?', a: 'Most interfaces support both button and keyboard entry.' },
      { q: 'Does clear reset all state?', a: 'Depends on mode; some calculators separate clear-entry and all-clear.' },
      { q: 'Can negative numbers be entered directly?', a: 'Yes, sign toggles or unary minus usually handle this.' },
      { q: 'Is this suitable for accounting workflows?', a: 'For simple arithmetic yes; complex accounting may need specialized tools.' },
    ],
    sections: [
      { title: 'Arithmetic Foundation', content: 'Basic calculators provide fast, low-friction numeric operations.' },
      { title: 'Input Predictability', content: 'Consistent precedence and clear-state behavior improve trust.' },
      { title: 'Precision Awareness', content: 'Minor floating-point artifacts are normal in decimal computations.' },
      { title: 'Usability Focus', content: 'Simple interface design supports quick everyday calculations.' },
      { title: 'Scope Boundaries', content: 'Advanced symbolic or statistical tasks require specialized calculators.' },
    ],
  },
  'long-division-calculator': {
    howTo: 'Enter dividend and divisor to compute quotient, remainder, and optional step-by-step long-division breakdown.',
    formula: 'Dividend = Divisor * Quotient + Remainder, with 0 <= remainder < divisor (for positive divisor).',
    faqs: [
      { q: 'What is long division?', a: 'A structured method for dividing numbers by repeated place-value subtraction.' },
      { q: 'What are quotient and remainder?', a: 'Quotient is whole-number result; remainder is leftover part.' },
      { q: 'Can decimals be produced?', a: 'Yes, extending division beyond integer places yields decimal quotient.' },
      { q: 'How to verify division result?', a: 'Multiply divisor by quotient and add remainder.' },
      { q: 'Can negative numbers be divided?', a: 'Yes, sign follows arithmetic rules while magnitude uses same process.' },
      { q: 'What if divisor is larger than dividend?', a: 'Quotient may be zero with remainder or decimal less than one.' },
      { q: 'Why show step-by-step output?', a: 'It helps learning and error tracing in manual division practice.' },
      { q: 'Is remainder form same as fractional form?', a: 'Remainder can be converted into fractional/decimal continuation.' },
      { q: 'Can this assist classroom homework?', a: 'Yes, especially when explanatory steps are included.' },
      { q: 'Does this replace conceptual learning?', a: 'No, it supports but does not replace understanding.' },
    ],
    sections: [
      { title: 'Place-Value Process', content: 'Long division decomposes division into manageable digit-level steps.' },
      { title: 'Result Integrity Check', content: 'Quotient-remainder identity validates correctness quickly.' },
      { title: 'Educational Transparency', content: 'Step output improves comprehension and debugging.' },
      { title: 'Remainder Interpretation', content: 'Remainders can be left as-is or converted to decimals/fractions.' },
      { title: 'Skill Building', content: 'Repeated structured practice strengthens arithmetic fluency.' },
    ],
  },
  'average-calculator': {
    howTo: 'Enter a set of numbers to compute average values such as arithmetic mean and optional weighted average.',
    formula: 'Arithmetic mean = sum(values) / count; weighted mean = sum(weight*value) / sum(weights).',
    faqs: [
      { q: 'What is average?', a: 'Most commonly it refers to arithmetic mean of a dataset.' },
      { q: 'Can weighted average be calculated?', a: 'Yes, when each value has an associated importance weight.' },
      { q: 'How does outlier affect average?', a: 'Extreme values can shift mean significantly.' },
      { q: 'Is average same as median?', a: 'No, median is middle sorted value and may differ from mean.' },
      { q: 'Can negative and decimal inputs be used?', a: 'Yes, all real-number values are typically valid.' },
      { q: 'When should weighted average be preferred?', a: 'When contributions are unequal, such as grades with category weights.' },
      { q: 'Can running average be tracked?', a: 'Yes, incremental updates can compute evolving average over time.' },
      { q: 'Does sample size matter?', a: 'Yes, larger samples often provide more stable averages.' },
      { q: 'Can average hide variation?', a: 'Yes, pair with spread metrics for fuller interpretation.' },
      { q: 'Is this inferential statistics?', a: 'No, it is descriptive summary computation.' },
    ],
    sections: [
      { title: 'Central Tendency Basics', content: 'Average provides a quick center estimate of numeric data.' },
      { title: 'Weighting Importance', content: 'Weighted means better represent unequal contribution scenarios.' },
      { title: 'Outlier Sensitivity', content: 'Mean should be interpreted with awareness of extreme values.' },
      { title: 'Complementary Metrics', content: 'Median and variance add essential context beyond average alone.' },
      { title: 'Practical Use', content: 'Common for grades, performance tracking, and planning summaries.' },
    ],
  },
  'p-value-calculator': {
    howTo: 'Enter test statistic and distribution context (z, t, chi-square, etc.) to estimate p-value for hypothesis testing.',
    formula: 'p-value is tail probability of observing statistic at least as extreme under null hypothesis.',
    faqs: [
      { q: 'What is p-value?', a: 'Probability of seeing data this extreme if null hypothesis were true.' },
      { q: 'Is p-value probability null is true?', a: 'No, it is conditional on null, not direct probability of null truth.' },
      { q: 'What does small p-value indicate?', a: 'Data appears less compatible with null under test assumptions.' },
      { q: 'Does p < 0.05 prove practical importance?', a: 'No, statistical significance is not the same as practical effect size.' },
      { q: 'Can one-tailed vs two-tailed change p?', a: 'Yes, test direction specification affects p-value computation.' },
      { q: 'Does larger sample always reduce p-value?', a: 'Larger samples can detect smaller effects, often lowering p-values.' },
      { q: 'Should confidence intervals be reported too?', a: 'Yes, intervals provide effect-size uncertainty context.' },
      { q: 'Can p-hacking distort conclusions?', a: 'Yes, selective analysis inflates false-positive risk.' },
      { q: 'Is p-value alone enough for decisions?', a: 'No, combine with design quality, assumptions, and effect size.' },
      { q: 'Can assumptions invalidate p-value?', a: 'Yes, violated model assumptions can make p-values misleading.' },
    ],
    sections: [
      { title: 'Null-Model Compatibility', content: 'P-value quantifies extremeness relative to null-distribution expectations.' },
      { title: 'Interpretation Guardrails', content: 'Avoid treating p-value as direct truth probability for hypotheses.' },
      { title: 'Tail Specification', content: 'One-sided vs two-sided framing materially changes results.' },
      { title: 'Evidence Context', content: 'Effect size and confidence intervals should accompany p-values.' },
      { title: 'Methodological Integrity', content: 'Pre-registered analysis and assumption checks improve reliability.' },
    ],
  },
  'mortgage-calculator': {
